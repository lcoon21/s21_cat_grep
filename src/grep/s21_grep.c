...
void s21_grep(t_flags_grep *flags, int file_numbers, char* find_str, FILE* fp, char* filename) {
    ...
    regmatch_t pmatch[2];  // структура для того чтоб отслеживать совпадения по флагу о
    while ((read = getline(&line, &len, fp)) != EOF) {
        char* line_tmp = calloc(strlen(line), sizeof(char));
        if (line[strlen(line)] == '\n' && strlen(line) == 1) // тут я подготавливаю строку
            s21_strcat(line, "\n");
        if (line[strlen(line) - 1] != '\n')
            s21_strcat(line, "\n");
        if (flags->i) { 
            s21_strcpy(line_tmp, line); // копирую во временную лайн
            char* line_tmp2 = s21_to_lower(line_tmp); // привожу  в лайн с нимжним регистром
            reti = regexec(&regex, line_tmp2, nmatch, pmatch, 0);  // рети - 0 и 1 (нашёл) в нём хранится результат было совпадение или нет
            free(line_tmp2);
        } else {
        ...
        if (flags->o && !flags->v && !reti) {
            if (strlen(line) != 1) {
                ...
                snprintf(line, strlen(line), "%.*s", (int)(pmatch[1].rm_eo - pmatch[1].rm_so), // rm eo там хрнанится указатель на конец вхождения в строку, а в so начало вхождения в строку
    ...
    flags_vcl(flags, count_c, count_vc, filename);  // для флагов в с л
    if (line) free(line);
    regfree(&regex);
}
